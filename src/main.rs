...
fn main() {
    ...
    let matches = App::new("RMI Learner for P4")
        .version("0.1")
        .author("Ryan Marcus <ryan@ryanmarc.us> adapted by Lucas Bürgi <lucas.buergi@unifr.ch>")
        .about("Learns recursive model indexes and generates P4 source code files")
        .arg(Arg::with_name("input")
             .help("Path to input file containing data")
             .index(1).required(true))
        .arg(Arg::with_name("namespace")
             .help("Name to use for generated files and generated code")
    ...
    let num_threads = matches.value_of("threads")
        .map(|x| x.parse::<usize>().unwrap()).unwrap_or(4);
    ...
    if matches.is_present("optimize") {
        ...
        let grid_specs: Vec<JsonValue> = results.into_iter().enumerate().map(|(idx, v)| {
    ...
    if !Path::new(data_dir).exists() {
        ...
        std::fs::create_dir_all(data_dir).expect("The RMI data directory did not exist, and it could not be created.");
    }
    ...
    if let Some(param_grid) = matches.value_of("param-grid").map(|x| x.to_string()) {
        ...
        if let JsonValue::Array(v) = pg {
            for el in v {
                ...
                let namespace = match el["namespace"].as_str() {
                    Some(s) => Some(String::from(s)), None => None
                };
            ...
            pbar.set_style(ProgressStyle::default_bar().template("{pos} / {len} ({msg}) {wide_bar} {eta}"));

            let train_func = |(models, branch_factor, namespace): &(String, u64, Option<String>)| {
                    ...
                    if let Some(nmspc) = namespace {
                        rmi_lib::output_rmi(&nmspc, trained_model, data_dir, key_type).unwrap();
                    }
        ...
        let mut trained_model = match matches.value_of("max-size") {
            None => {
                ...
                let branch_factor = matches.value_of("branching factor").unwrap().parse::<u64>().unwrap();

                let trained_model = dynamic!(train, data, models, branch_factor);
                trained_model
            }
            Some(max_size_str) => {
        ...
        info!("Average model error: {} ({}%)", trained_model.model_avg_error as f64,
            trained_model.model_avg_error / num_rows as f64 * 100.0);

        info!("Average model L2 error: {}", trained_model.model_avg_l2_error);
        info!("Average model log2 error: {}", trained_model.model_avg_log2_error);
        info!("Max model log2 error: {}", trained_model.model_max_log2_error);
        info!("Max model error on model {}: {} ({}%)",
        ...
        if !matches.is_present("no-code") {
            ...
            rmi_lib::output_rmi(&namespace, trained_model, data_dir, key_type).unwrap();
        } else {
